...
export function getHighestOrderIndex(array) {
  ...
  const highestOrderIndex = array.reduce((accumulator, currentValue) => {
    if (currentValue[1]["order_index"] > accumulator) {
      return currentValue[1]["order_index"];
    }

    return accumulator;
  }, array[0][1]["order_index"]);
...
export function getNextOrderIndex(key, array) {
  ...
  const nextOrderIndex = array.reduce((accumulator, currentValue) => {
    if (accumulator === "") {
      if (currentValue[1]["order_index"] > currentItemOrderIndex) {
        return currentValue[1]["order_index"];
      } else {
        return accumulator;
      }
    ...
    if (
      currentValue[1]["order_index"] < accumulator &&
      currentValue[1]["order_index"] > currentItemOrderIndex
    ) {
      return currentValue[1]["order_index"];
    }

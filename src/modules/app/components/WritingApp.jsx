    ...
    <DndProvider backend={HTML5Backend}>
      <AnimatePresence mode="sync">
        <ContextMenuWrapper options={windowOptions}>
          <motion.div
            id="Layout"
            className={`h-screen w-screen max-w-screen min-w-screen max-h-screen min-h-screen bg-appBackgroundAccent overflow-hidden font-[NotoSans] w400  border-appLayoutBorder text-appLayoutText
            ${!isMaximized ? "border border-r-2 border-appLayoutInverseHover border-b-2 rounded-2xl" : "rounded-none"}
            `}
          >
            {loading && (
              <LoadingScreen loading={loading} loadingStage={loadingStage} />
            )}

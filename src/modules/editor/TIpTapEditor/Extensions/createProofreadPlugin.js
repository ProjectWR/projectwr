      ...
      tasks.push(async () => {
        ...
        if (node.textContent && node.textContent.length > 1) {
          ...
          if (!pluginState.cacheMap.has(nodeKey)) {
            ...
            const errors = await proofread(
              getCustomText ? getCustomText(node) : getDefaultCustomText(node),
            );
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, old, oldState, newState) {
        ...
        if (
          !tr.docChanged &&
          spellcheckEnabled === old.spellcheckEnabled &&
          !forceProofread
        )
          return old;
...
function getDefaultCustomText(node) {
  ...
  node.content.forEach((child) => {
    if (child.isText) {
      ...
      textContent += `\x1F${child.attrs.label}\x1F`;
    } else if (child.isInline) {
      textContent += `\x1F${getDefaultCustomText(child)}\x1F`;
    } else {
